...
<head>
  ...
  .card-img-top:hover{
    width: 300px;
    transition: 0.9s;
    -webkit-transition: border-radius 10;
  }
</style>
...
<body class="grey lighten-3">
  ...
  <?php 
  $i=1;
  $sql="SELECT * FROM `products` INNER JOIN `stocks` ON PRODUCTS.product_id=STOCKS.product_id INNER JOIN PRODUCT_IMAGE ON PRODUCTS.product_id = PRODUCT_IMAGE.product_id";
  ...
  if($cek>0){
    ...
    while($data=mysqli_fetch_array($query)){
      ?>
       <tr onclick="window.location='productdetails.php?pid=<?php echo  $data["product_id"];?>';">
            <td><img src='data:image/jpeg;base64,<?php echo base64_encode($data['image']); ?>' class='card-img-top'/></td>
            <td><?php echo $data['product_name']; ?></td>
            <td><?php echo $data['product_category']; ?></td>
            <td><?php echo $data['in_stock']; ?></td>
            <td><?php echo $data['sell_price']; ?></td>
            <td style="font-weight: bold;"><a href="action.php?deleteproduct=delete&pid=<?php echo  $data['product_id'];?>" style="color: red;">Delete</a> <a href="modifyproduct.php?pid=<?php echo  $data['product_id'];?>" style="color: rgb(3, 148, 252);">Modify</a></td>
          </tr>
      <?php 
      } }
      ?>
