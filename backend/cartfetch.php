...
IF( isset( $_SESSION['name'] ) ) {
    ...
    $query_for_cart = "SELECT PRODUCTS.product_id, PRODUCTS.product_name,PRODUCTS.sell_price, CART.product_id, CART.quantity, CART.cust_id FROM CART INNER JOIN PRODUCTS ON PRODUCTS.product_id=CART.product_id AND CART.cust_id='$cust_id'";
    ...
    if ( mysqli_num_rows( $has_value ) > 0 ) {
        while( $row = mysqli_fetch_array( $has_value ) ) {
            ...
            $output .= '<tr>
                          ...
                          <td> ৳ '.$product_price.'</td>
                          <form>
                          <td><input type="text" id="qty" value="'.$qty.'" class="input-quantity" data-qty="'.$qty.'"</td>
                          <td>
                            <div class="btn-group" role="group" aria-label="Basic example">
                               <button type="button" class="btn btn-primary" id="update" data-userid="'.$cust_id.'" data-product="'.$product_id.'"><i class="fa fa-pencil-square-o"></i> update</button>
                               <button type="button" class="btn btn-danger" id="delete" data-userid="'.$cust_id.'" data-product="'.$product_id.'"><i class="fa fa-trash-o"></i> delete</button>
                            </form>
                            </div>
                          </td>
                        </tr>';
